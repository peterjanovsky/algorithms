...
void test_linked_list_insert()
{
    ...
    node_t *node = linked_list_create();
    node->key = key;
    node->value = value;

    linked_list_node_insert(node, "bar", "bar-value");
    linked_list_node_insert(node, "baz", "baz-value");

    int counter = 0;
    while (node != NULL) {
        printf("key: %s\n", node->key);
        if (counter == 0) assert(strcmp(node->key, "foo") == 0);
        else if (counter == 1) assert(strcmp(node->key, "bar") == 0);
        else assert(strcmp(node->key, "baz") == 0);
        counter++;
        node = node->next;
    }
}
